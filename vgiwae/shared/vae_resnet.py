...
class ResNetEncoder(nn.Module):
    def __init__(self, input_shape: Tuple[int, int, int], layers: List[int], layer_widths: List[int],
                 latent_dim: int, first_conv: bool=False, maxpool1: bool=False, dropout_prob:float=0.,
                 expect_miss_mask_as_extra_chennel: bool=False):
        super().__init__()
        ...
        if expect_miss_mask_as_extra_chennel:
    ...
    def add_mask_inputs_to_existing_network(self):
        num_input_channels = self.conv1.in_channels + 1
        out_channels = self.conv1.out_channels

        if self.first_conv:
            conv1_new = nn.Conv2d(num_input_channels, out_channels, kernel_size=7, stride=2, padding=3, bias=False)
        else:
            conv1_new = nn.Conv2d(num_input_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)

        conv1_new.weight.data[:, :self.conv1.in_channels, :, :] = self.conv1.weight.data
    ...
    def forward(self, x, m=None):
        ...
        if m is not None:
            # Concatenate the missingness mask to the channel dimension
            m = m.view(-1, 1, *self.input_shape[1:])
            x = torch.cat((x, m), dim=-3)
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, output_shape: Tuple[int, int, int], layers: List[int], layer_upscale: List[Optional[int]],
                 layer_widths: List[int], downscaled_resolution: Tuple[int, int], latent_dim: int, upscale_factor: float,
        ...
        self.upscale1 = Interpolate(size=int(self.input_height // self.upscale_factor))
